...
# ── KPI / feature-card pattern detectors ──────────────────────────────────────

def _is_kpi_value(text: str) -> bool:
    """
    Detect a short metric/KPI value para: ~1 Tbps, 10M/km², 2030s, 289%, etc.

    Rules (all must pass):
    - At most 25 characters and at most 4 words
    - Either starts with a modifier+digit (~, >, <, ≥, ≤, +) followed by a digit,
      OR starts with a bare digit and any second word is a short unit (≤5 chars),
      OR is a 4-digit year (optionally followed by 's').
    """
    t = text.strip()
    if not t or len(t) > 25 or len(t.split()) > 4:
        return False
    # Modifier + digit: ~1 Tbps, >1 Tbps, ≥10M
    if re.match(r'^[~>≥≤<+]\d', t):
        return True
    # Year: 2030s, 2025
    if re.match(r'^\d{4}s?$', t):
        return True
    # Bare digit start: "1 Tbps", "10M/km²", "201 TWh", "289%", "60 fps"
    # Second word (if any) must be a short unit abbreviation (≤5 chars)
    words = t.split()
    if words and words[0][0].isdigit():
        if len(words) == 1:
            return True
        unit = words[1]
        if len(unit) <= 5 and re.match(r'^[A-Za-z%°²³×/][\w²³]*$', unit):
            return True
    return False
...
def _is_kpi_label(text: str) -> bool:
    """
    Detect a KPI label para: short (1-6 words), no trailing period, title-cased.

    Used to pair with _is_kpi_value() to identify KPI pairs in the block stream.
    """
    t = text.strip()
    words = t.split()
...
def _is_card_description(text: str) -> bool:
    """Detect a feature card description: long prose sentence (> 65 chars, > 9 words)."""
    return len(text) > 65 and text.count(' ') > 9


def _is_card_badge(text: str) -> bool:
    """
    Detect a short feature card badge/tag para: 1-4 words, ≤32 chars, no period,
    not a KPI metric value, not an HTML entity or star glyph.
    """
    t = text.strip()
    words = t.split()
    return (
...
def parse_blocks(lines: list[str]) -> list[dict]:
    ...
    while i < len(lines):
        ...
        if line.startswith("### "):
            ...
            _flush_para()
            # Collect table rows.  Handles both:
            #   - Standard Markdown (header | separator | data rows, consecutive)
            #   - Non-standard format (blank lines between rows, no separator row)
            # Separator rows (|---|---| patterns) are stripped so render_sortable_table
            # always receives [header_row, data_row, data_row, ...].
            tbl: list[str] = []
            k = i
            while k < len(lines):
                cur = lines[k].strip()
                if cur == "":
                    # Blank line: peek ahead to see if a | row follows
                    peek = k + 1
                    while peek < len(lines) and lines[peek].strip() == "":
                        peek += 1
                    if peek < len(lines) and lines[peek].strip().startswith("|"):
                        k = peek   # jump over blanks to the next | row
                        continue
                    else:
                        break      # no more | rows — table ends here
                elif cur.startswith("|"):
                    # Keep data/header rows; drop pure separator rows (|---|---|)
                    if not re.match(r'^\s*\|[-| :]+\|\s*$', lines[k]):
                        tbl.append(lines[k])
                    k += 1
                else:
                    break
            if len(tbl) >= 2:
                ...
                i = k
                continue
            # Fallback: single | line or ambiguous — treat as paragraph
    ...
    _flush_para()
    raw = [b for b in blocks if (
        b.get("content") or b.get("items") or b.get("lines") or
        b.get("src") or b["type"] == "code"
    )]
    return _postprocess_blocks(raw)


def _postprocess_blocks(blocks: list[dict]) -> list[dict]:
    """
    Merge consecutive paragraph blocks into semantic widget blocks.

    Two passes over the block list:
    1. KPI pass  — consecutive (label_para + value_para) pairs → kpi_row
    2. Card pass — consecutive (title_para + desc_para [+ badge_para]) ≥ 2 → card_grid

    Non-para blocks are passed through unchanged.  Unmatched para blocks are
    preserved as-is so render_blocks_html can group them into prose-cards.
    """
    result: list[dict] = []
    i = 0
    n = len(blocks)
    ...
    while i < n:
        ...
        j = i
        while (j + 1 < n
               and blocks[j]["type"] == "para"
               and blocks[j + 1]["type"] == "para"
               and _is_kpi_label(blocks[j]["content"])
               and _is_kpi_value(blocks[j + 1]["content"])):
            kpi_items.append({
                "label": blocks[j]["content"].strip(),
                "value": blocks[j + 1]["content"].strip(),
            })
            j += 2

        if kpi_items:
            result.append({"type": "kpi_row", "items": kpi_items})
            i = j
            continue

        # ── Pass 2: Feature card triples (title + description [+ badge]) ─────
        card_items: list[dict] = []
        j = i
        while j < n and blocks[j]["type"] == "para":
            title = blocks[j]["content"].strip()
            title_words = title.split()
            if not (1 <= len(title_words) <= 6 and not title.endswith('.')):
                break
            if j + 1 >= n or blocks[j + 1]["type"] != "para":
                break
            desc = blocks[j + 1]["content"].strip()
            if not _is_card_description(desc):
                break
            nxt = j + 2
            badge = ""
            if (nxt < n
                    and blocks[nxt]["type"] == "para"
                    and _is_card_badge(blocks[nxt]["content"])):
                badge = blocks[nxt]["content"].strip()
                nxt += 1
            # Compose card text as "**Title** description (badge)"
            full = f"**{title}** {desc}"
            if badge:
                full += f"  \n_{badge}_"
            card_items.append({"text": full, "depth": 0, "checked": None})
            j = nxt

        if len(card_items) >= 2:
            result.append({"type": "card_grid", "items": card_items})
            i = j
            continue

        # No pattern matched — preserve the para block as-is
        result.append(b)
        i += 1
...
def render_checklist(items: list[dict], uid: str, strings: dict | None = None) -> str:
    ...
    progress_tpl = strings.get("checklist_progress", "{done} of {total} completed")
    label_text = progress_tpl.replace("{done}", str(done)).replace("{total}", str(total))
  ...
  <div class="checklist-ring-wrap">
    ...
    <div class="ring-label">
      ...
      <span data-i18n="checklist_progress" data-done="{done}" data-total="{total}">{label_text}</span>
    </div>
  </div>
...
def render_sortable_table(table_lines: list[str], uid: str) -> str:
    ...
    # table_lines[0] is the header row; all remaining rows are data rows.
    # Separator rows (|---|---|) are already stripped by parse_blocks().
    data_rows = [r for r in table_lines[1:] if "|" in r]
    if data_rows:
...
def render_kpi_row(items: list[dict]) -> str:
    """Render a list of KPI label+value pairs as a responsive .kpi-row card row."""
    cards = []
    for it in items:
        label = _html_lib.escape(it.get("label", ""))
        value = _html_lib.escape(it.get("value", ""))
        cards.append(
    ...
    return f'<div class="kpi-row widget">\n{"".join(cards)}\n</div>'


def render_tree(items: list[dict], uid: str, strings: dict | None = None) -> str:
    """Render a nested bullet list as an expandable tree with collapse controls."""
    if strings is None:
        strings = {}
    expand_label  = strings.get("expand", "Expand")
    collapse_label = strings.get("collapse", "Collapse")
    top = [it for it in items if it["depth"] == 0]
    ...
    return (
        ...
        f'<button class="tree-ctrl-btn" data-i18n="expand" onclick="expandAllTree(\'{uid}\')">{expand_label}</button>'
        f'<button class="tree-ctrl-btn" data-i18n="collapse" onclick="collapseAllTree(\'{uid}\')">{collapse_label}</button>'
        f'</div>'
...
def render_widget_block(block: dict, uid: str, strings: dict | None = None) -> str:
    """Dispatch a typed content block to the appropriate widget renderer."""
    t = block["type"]
    if t == "kpi_row":
        return render_kpi_row(block["items"])
    if t == "highlight":
        return render_highlight_list(block["items"])
    if t == "card_grid":
        return render_card_grid(block["items"])
    if t == "checklist":
        return render_checklist(block["items"], uid, strings)
    if t == "def_list":
        return render_def_list(block["items"])
    if t == "tree":
        return render_tree(block["items"], uid, strings)
    if t == "table":
...
def render_blocks_html(
    ...
    Args:
        ...
        strings:     Optional i18n string map (from i18n.json) for widget
                     chrome localisation (checklist labels, tree buttons, etc.).
    ...
    for block in blocks:
        if block["type"] in PROSE_TYPES:
            ...
            _flush_prose()
            uid = _next_uid()
            parts.append(render_widget_block(block, uid, strings))
