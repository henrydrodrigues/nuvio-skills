...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
    ...
    } else if (el.type === 'table-native') {
      const pptxRows = el.rows.map(row =>
        row.map(cell => {
          const cellOpts = {
            fill: cell.fill,
            color: cell.color,
      ...
      targetSlide.addTable(pptxRows, {
        ...
        y: el.position.y,
        w: el.position.w,
        border: { type: 'solid', color: 'E8EEEF', pt: 0.5 },
      });
    } else {
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const rgbToHex = (rgbStr, parentBg) => {
      // Handle transparent backgrounds by defaulting to white
      if (rgbStr === 'rgba(0, 0, 0, 0)' || rgbStr === 'transparent') return parentBg || 'FFFFFF';

      const match = rgbStr.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)/);
      if (!match) return parentBg || 'FFFFFF';
      let [r, g, b] = match.slice(1, 4).map(Number);
      const a = match[4] !== undefined ? parseFloat(match[4]) : 1;

      // Blend rgba against parent background when alpha < 1
      if (a < 1 && parentBg) {
        const pr = parseInt(parentBg.slice(0, 2), 16);
        const pg = parseInt(parentBg.slice(2, 4), 16);
        const pb = parseInt(parentBg.slice(4, 6), 16);
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      // Skip elements marked with data-pptx-skip (e.g. opaque video placeholders)
      if (el.hasAttribute('data-pptx-skip')) {
      ...
      // Extract TABLE elements — parsed into native pptxgenjs addTable() data
      if (el.tagName === 'TABLE') {
        const rect = el.getBoundingClientRect();
        if (rect.width > 0 && rect.height > 0) {

          function extractRuns(cell) {
            const runs = [];
            function walk(node) {
              if (node.nodeType === 3) {
                const t = node.textContent;
                if (t) runs.push({ text: t, options: {} });
                return;
              }
              ...
              if (c && c !== '000000') opts.color = c;
              if (parseInt(s.fontWeight, 10) >= 600) opts.bold = true;
              const ff = s.fontFamily.split(',')[0].replace(/['"]/g, '').trim();
              if (ff) opts.fontFace = ff;
              const prevLen = runs.length;
              node.childNodes.forEach(walk);
              for (let i = prevLen; i < runs.length; i++) {
                runs[i].options = Object.assign({}, opts, runs[i].options);
              }
            }
            cell.childNodes.forEach(walk);
            const hasFormatting = runs.some(r => Object.keys(r.options).length > 0);
            return hasFormatting ? runs : runs.map(r => r.text).join('');
          }

          // Resolve table's parent background for rgba blending
          let tableBg = 'FFFFFF';
          let ancestor = el.parentElement;
          while (ancestor) {
            const acs = window.getComputedStyle(ancestor);
            const abg = acs.backgroundColor;
            if (abg && abg !== 'transparent' && abg !== 'rgba(0, 0, 0, 0)') {
              tableBg = rgbToHex(abg);
              break;
            }
          ...
          el.querySelectorAll('tr').forEach(tr => {
            // Resolve TR/THEAD/TBODY background (CSS background on <tr> doesn't
            // propagate to getComputedStyle(cell).backgroundColor — cells stay
            // transparent even though the row visually has a background).
            let rowBg = tableBg;
            for (const rowAncestor of [tr, tr.parentElement]) {
              if (!rowAncestor) continue;
              const rcs = window.getComputedStyle(rowAncestor);
              const rbg = rcs.backgroundColor;
              if (rbg && rbg !== 'transparent' && rbg !== 'rgba(0, 0, 0, 0)') {
                rowBg = rgbToHex(rbg, tableBg);
                break;
              }
            }
            ...
            tr.querySelectorAll('th, td').forEach(cell => {
              ...
              if (cell.colSpan > 1) cellObj.colspan = cell.colSpan;
              if (cell.rowSpan > 1) cellObj.rowspan = cell.rowSpan;
              cells.push(cellObj);
            });
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    try {
      ...
      // Convert SVG images to PNG (PptxGenJS doesn't support SVG)
      const svgElements = slideData.elements.filter(
        e => e.type === 'image' && e.src && e.src.toLowerCase().endsWith('.svg')
      );
      if (svgElements.length > 0) {
        ...
        for (const svgEl of svgElements) {
          for (let i = 0; i < imgCount; i++) {
            const handle = imgLocators.nth(i);
            const resolvedSrc = await handle.evaluate(el => el.src);
            if (resolvedSrc === svgEl.src) {
              const pngPath = path.join(tmpDir, `svg-${Date.now()}-${i}.png`);
      ...
      // Convert SVG background images to PNG
      if (slideData.background.type === 'image' && slideData.background.path &&
          slideData.background.path.toLowerCase().endsWith('.svg')) {
        const bgPngPath = path.join(tmpDir, `svg-bg-${Date.now()}.png`);
        await page.screenshot({ path: bgPngPath });
        slideData.background.path = bgPngPath;
      }
    } finally {
      await browser.close();
    }
